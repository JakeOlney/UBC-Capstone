...
-(void) scrollPlot1:(NSTimer *)scrollTimer1
{
    ...
    else
    {
        if (ECG)
        {
            if(screenStart1 == plotIndex - 1100)
            {
                CPTXYPlotSpace *plotSpace = (CPTXYPlotSpace *)theGraph.defaultPlotSpace;
...
-(void)scatterPlot:(CPTScatterPlot *)plot plotSymbolWasSelectedAtRecordIndex:(NSUInteger)idx
{
    ...
    /*else
    {
        for(int i = idx-10; i < idx+10; i++)
        {
            if([data2 objectAtIndex:i] < [data2 objectAtIndex:i+1])
            {
                minTemp = [data2 objectAtIndex:i];
                
                if([data2 objectAtIndex:i] < min)
                {
                    min = [data2 objectAtIndex:i];
                    minIndex = i;
                }
            }
